...
import type { ChatRequest, ChatResponse, ChatResponseAction } from "@/lib/types";
...
async function callBackendAPI(chatRequest: ChatRequest): Promise<ChatResponse> {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Backend API error: ${response.status} - ${errorData.detail || errorData.message || "Unknown error from backend"}`,
      );
    }
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(
      {
        message:
          "Sorry, I have encountered a technical problem. Retry later or contact Lorenzo directly using the form",
